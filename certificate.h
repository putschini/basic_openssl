...
namespace certificate {
    ...
    void creat_private_key( std::string certificate_file, std::string password, std::string private_key_file ) {
        EVP_PKEY* private_key;
        X509 *certificate;
        STACK_OF(X509) *ca;

        read_pk12_certificate( certificate_file, password, &private_key, &certificate, &ca );

        BIO*  pembio = BIO_new_file(private_key_file.c_str(), "w");
        if( pembio == NULL ) 
            utils::print_error_exit( "Error opening file " + private_key_file );

        if( !PEM_write_bio_PKCS8PrivateKey( pembio, private_key, EVP_des_ede3_cbc(), (char*)password.c_str(), password.length(), 0, NULL ) )
    ...
    void creat_public_key( std::string certificate_file, std::string password, std::string pub_key_file ) {
        EVP_PKEY* private_key;
        X509 *certificate;
        STACK_OF(X509) *ca;
    ...
    EVP_PKEY* read_private_key_form_certificate( std::string certificate_file, std::string password ) {
        EVP_PKEY* private_key;
        X509 *certificate;
